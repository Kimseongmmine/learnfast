...
const PROFILE_FIELDS = [
  ...
  { key: "prefs", label: "선호·비선호", ph: "예: 아침 일찍은 싫음 / 카페에서 집중 잘됨 / 저녁은 가볍게" },
  ...
  { key: "places", label: "자주 가는 장소 · 이동 시간", ph: "예: 학교 도서관 25분 / 집 앞 카페 5분 / 집 책상 / 체육관 15분" }
];

// ---- 학기 시험 일정 ----
// 마감으로 도는 기능이 셋이다: 과목 우선순위 · 복습 비중 · 시험 역산.
// 과목마다 날짜를 안 치면 셋 다 죽으므로, 학기 시험을 하나 두고 목표가 물려받는다.
// 기본값은 두지 않는다 — 학교마다 학기가 달라서 남의 날짜는 틀린 정보다. 온보딩에서 받는다.
...
function loadProfile() {
  ...
  // 기본값을 채우지 않는다. 남의 대학 이름과 남의 시간표는 도움이 아니라 오해다.
  // 비어 있으면 온보딩(renderOnboard)이 뜬다.
  if (p.courses === undefined) p.courses = "";
  if (p.exams === undefined) p.exams = "";
  if (p.places === undefined) p.places = "";
  return p;
}
...
// 어려운 과목은 사용자가 목표 카드에서 직접 표시한다(goal.hard). 과목명으로 짐작하지 않는다.
...
function courseScore(goal, today, profile) {
  ...
  if (goal.hard) sc += 15;                                        // 사용자가 어렵다고 표시한 과목
...
async function aiGeneratePlan(candidates, targetDate, opts) {
  ...
  const parse = function (c) {
    ...
    if (j && Array.isArray(j.blocks) && j.blocks.length) return { blocks: j.blocks };
    if (j && Array.isArray(j.schedule) && j.schedule.length) return { blocks: j.schedule };
    const t = String(c || "").trim();
    if (t[0] === "[") {
      try { const arr = JSON.parse(t); if (Array.isArray(arr) && arr.length && arr[0] && arr[0].time) return { blocks: arr }; } catch (e) {}
    }
    const text = parseTextSchedule(c, candidates);
    return text ? { blocks: text } : null;
  };
...
async function generatePlan(targetDate, opts) {
  ...
  try {
    ...
    if (aiOn) {
      ...
      if (res) { blocks = fillPlaces(res.blocks, loadProfile()); source = "ai"; }
    }
    if (!blocks) {
      const prof = loadProfile();
...
function renderGuide() {
  ...
  [
    ...
    { icon: "📈", name: "기록 · 측정", what: "최근 7일 착수율, 복습이 얼마나 굳었는지",
...
function renderFlow() {
  ...
  box.id = "flow";
  box.setAttribute("aria-label", "기록");
  box.appendChild(el("h2", { text: "최근 7일" }));
  box.appendChild(el("p", { cls: "what", text: "하루가 아니라 흐름을 봅니다. 어느 요일이 약한지, 배터리가 어땠는지." }));

  const s2 = recentStats(today, 7);
  const short = el("div", { cls: "layer" });
  if (s2.pct == null) {
    short.appendChild(el("div", { cls: "muted", text: "아직 기록이 없습니다. 계획을 만들고 블록을 시작하면 여기에 쌓입니다." }));
  } else {
    short.appendChild(el("div", { cls: "bignum", text: "착수 " + s2.onTime + "/" + s2.coreTotal + " (" + s2.pct + "%)" + (s2.late ? ("  · 늦음 " + s2.late) : "") }));
    if (s2.battery) short.appendChild(el("div", { cls: "muted", text: s2.battery }));
    if (s2.worstDay) short.appendChild(el("div", { cls: "muted", text: s2.worstDay.day + "이 가장 약함 — 착수 " + s2.worstDay.onTime + "/" + s2.worstDay.core }));
  }
  const stall = stalledTask(loadProfile());
  if (stall) short.appendChild(el("div", { cls: "muted", text: "가장 오래 밀린 것 · " + stall.text + " (" + stall.goalTitle + ")" }));
